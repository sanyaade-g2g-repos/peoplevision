...
void ofxTUIOSender::cursorPressed(float x, float y, int cursorId) {
	myCursor[cursorId].x      = x;
	myCursor[cursorId].y      = y;
	myCursor[cursorId].isAlive   = true;
}

void ofxTUIOSender::cursorDragged(float x, float y, int cursorId) {
	myCursor[cursorId].x      = x;
	myCursor[cursorId].y      = y;
	myCursor[cursorId].isAlive   = true;
...
void ofxTUIOSender::cursorReleased(float x, float y, int cursorId) {
	myCursor[cursorId].x      = x;
	myCursor[cursorId].y      = y;
	myCursor[cursorId].isAlive   = false;
}

void ofxTUIOSender::setup(std::string host, int port) {
