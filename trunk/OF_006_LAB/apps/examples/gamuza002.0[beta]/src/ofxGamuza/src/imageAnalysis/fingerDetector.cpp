...
bool fingerDetector::findHands(ofxCvBlob smblob)
{
	...
	for(unsigned int i=0; i<smppico.size();i++)
	{
		if(i==0)
		{
	...
	if(lhand.size()>0)
	{
		...
		max=lhd;
		handspos[0]=0;
		for(unsigned int i=1; i<lhand.size(); i++)
		{
	...
	if(rhand.size()>0)
	{
		...
		max=lhd;
		handspos[1]=0;
		for(unsigned int i=1; i<rhand.size(); i++)
		{
...
{
	for(unsigned int i=0; i<ppico.size(); i++)
	{
...
int fingerDetector::getTotFingers(){
	return ppico.size();
}
