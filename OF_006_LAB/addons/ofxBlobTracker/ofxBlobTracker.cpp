...
ofxBlobTracker::ofxBlobTracker() {
	...
	currentID = 1;
	extraIDs = 0;
	reject_distance_threshold = 40;
	minimumDisplacementThreshold = 2.0f;
	ghost_frames = 6;
}

void ofxBlobTracker::draw( float x, float y, float scale ) {
 ...
 */
void ofxBlobTracker::trackBlobs( const vector<ofxBlob>& _blobs ) {
	unsigned int i, j, k;
	...
	for( i=0; i<_blobs.size(); i++ ) {
		blobs.push_back( ofxTrackedBlob(_blobs[i]) );
	}
	
	vector<ofxTrackedBlob> *prev = &history[history.size()-1];
	
	int cursize = blobs.size();
	int prevsize = (*prev).size();
	
	
	// figure out the 'error' (distance) to all blobs in the previous
	...
	// sort, make list of the closest blobs in the previous frame..
	for( i=0; i<cursize; i++ ) {
		// Bubble sort closest.
		for( j=0; j<prevsize; j++ )	{
	...
	if( best_error_ndx != -1 ) {
		for( i=0; i<cursize; i++ ) {
			...
			if( blobs[i].id != -1 ) {
				ofxTrackedBlob *oldblob = &(*prev)[matrix[best_error_ndx][i]];
...
//
//
void ofxBlobTracker::doBlobOn( const ofxTrackedBlob& b ) {
    if( listener != NULL ) {
        listener->blobOn( b );
    } else {
...
void ofxBlobTracker::doBlobMoved( const ofxTrackedBlob& b ) {
    if( listener != NULL ) {
...
void ofxBlobTracker::doBlobOff( const ofxTrackedBlob& b ) {
    if( listener != NULL ) {
        listener->blobOff( b );
    } else {
...
//
//
inline void ofxBlobTracker::permute( int start ) {  
    if( start == ids.size() ) {
...
inline bool ofxBlobTracker::checkValidNew( int start ) {
	int newidcount = 0;
	newidcount ++;
	for( int i=0; i<start; i++ ) {
        if(ids[i] == -1) {
...
inline bool ofxBlobTracker::checkValid( int start ) {
	for(int i=0; i<start; i++) {
