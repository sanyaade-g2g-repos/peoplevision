...
vector<int> ofxCvOpticalFlowLK::getFlowFromPoint(int x, int y){
	...
	pointVector.push_back(dx);
	pointVector.push_back(dy);
	return pointVector;
}

vector<vector <int> > ofxCvOpticalFlowLK::getFlowVector(){
	vector<vector <int> > flowVector;
	...
	for ( x = 0; x < captureWidth; x+=captureColsStep ){
		for ( y = 0; y < captureHeight; y+=captureRowsStep ){
			...
			pointVector.push_back(dx);
			pointVector.push_back(dy);
			flowVector.push_back(pointVector);
		}
	}
	return flowVector;
}
