...
void ofxMidiIn::manageNewMessage(double deltatime, std::vector< unsigned char > *message){
    ...
    if(nBytes>0){
        ...
        infoTest.port = (int)message->at(0);
        infoTest.channel = (int)message->at(1);
        infoTest.status = (int)message->at(2);
        infoTest.timestamp = deltatime;
...
// --------------------------------------------------------------------------------------
int ofxMidiIn::getMessageChannel(){
    return infoTest.channel;
}

// --------------------------------------------------------------------------------------
int ofxMidiIn::getMessageStatus(){
    return infoTest.status;
}
