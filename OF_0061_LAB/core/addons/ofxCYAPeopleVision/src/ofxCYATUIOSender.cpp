...
void ofxCYATUIOSender::cursorPressed(float x, float y, int cursorId) {
	myCursor[cursorId].x      = x;
	myCursor[cursorId].y      = y;
	myCursor[cursorId].isAlive = true;
}

void ofxCYATUIOSender::cursorDragged(float x, float y, int cursorId) {
	myCursor[cursorId].x      = x;
	myCursor[cursorId].y      = y;
	myCursor[cursorId].moved   = true;
}

void ofxCYATUIOSender::cursorReleased(float x, float y, int cursorId) {
	myCursor[cursorId].x      = x;
	myCursor[cursorId].y      = y;
	myCursor[cursorId].isAlive   = false;
}

void ofxCYATUIOSender::setup(std::string host, int port) {
